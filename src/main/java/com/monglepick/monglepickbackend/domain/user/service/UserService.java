...
import com.monglepick.monglepickbackend.domain.user.entity.UserRoleType;
...
public class UserService extends DefaultOAuth2UserService implements UserDetailsService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if (registrationId.equals(SocialProviderType.NAVER.name())) {
            ...
            // 네이버 응답에서 실제 유저 정보가 담긴 "response" 키를 꺼냄
            attributes = (Map<String, Object>) oAuth2User.getAttributes().get("response");

            // 이메일 추출
            useremail = attributes.get("email").toString();

            // 이름 추출 - 네이버 앱에서 이름 권한이 없으면 null로 올 수 있음
            // null이면 이메일 @ 앞부분을 닉네임으로 대신 사용 (예: test@naver.com → test)
            Object nameObj = attributes.get("name");
            usernickname = nameObj != null ? nameObj.toString() : useremail.split("@")[0];

        } else if (registrationId.equals(SocialProviderType.GOOGLE.name())) {

            // 구글은 attributes에 바로 유저 정보가 있음
        ...
        Optional<User> entity = userRepository.findByUserEmailAndIsSocialAndSocialProviderType(
                useremail, true, SocialProviderType.valueOf(registrationId)
        );

// 같은 이메일로 다른 방식(자체 or 다른 소셜)으로 이미 가입된 계정이 있는지 확인
        boolean emailAlreadyExists = userRepository.existsByUserEmail(useremail);
        if (emailAlreadyExists && entity.isEmpty()) {
            // 이미 다른 방식으로 가입된 이메일 → 에러 처리
        ...
        authorities = List.of(new SimpleGrantedAuthority("ROLE_"+UserRoleType.USER.name()));
