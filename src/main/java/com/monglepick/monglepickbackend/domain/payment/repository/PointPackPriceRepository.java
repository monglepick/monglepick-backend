...
public interface PointPackPriceRepository extends JpaRepository<PointPackPrice, Long> {
    ...
    /**
     * 결제 금액 + 지급 포인트 조합이 활성 가격표에 존재하는지 검증한다.
     *
     * <p>클라이언트 변조 방지: 요청의 (amount, pointsAmount)가 서버 가격표와 정확히 일치해야만 주문 생성을 허용.</p>
     *
     * @param amount       결제 금액 (KRW)
     * @param pointsAmount 지급 포인트
     * @return 일치하는 활성 가격 행 (없으면 Optional.empty → INVALID_POINT_PACK 에러)
     */
    Optional<PointPackPrice> findByAmountAndPointsAmountAndIsActiveTrue(Integer amount, Integer pointsAmount);
